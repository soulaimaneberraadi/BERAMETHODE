...
export default function App() {
    ...
    const [currentView, setCurrentView] = useState<'dashboard' | 'ingenierie' | 'atelier' | 'library' | 'coupe' | 'effectifs' | 'gestionRh' | 'planning' | 'suivi' | 'magasin' | 'export' | 'config' | 'parametres' | 'pageMachine' | 'profil' | 'admin' | 'rendement'>('dashboard');
    const [directSuiviModelId, setDirectSuiviModelId] = useState<string | null>(null);
    const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
    const defaultNavOrder = ['dashboard','ingenierie','library','coupe','effectifs','gestionRh','planning','suivi','rendement','magasin','export','config','parametres','pageMachine','admin'];
    ...
    useEffect(() => {
        const ALLOW = new Set(['dashboard', 'ingenierie', 'atelier', 'library', 'coupe', 'effectifs', 'gestionRh', 'planning', 'suivi', 'magasin', 'export', 'config', 'parametres', 'pageMachine', 'profil', 'admin', 'rendement']);
        const applyHash = () => {
    ...
    /** Pousse Machin vers la vue « Parc Machines » depuis Page Machine (lien inventaire). */
    const [machinParcSignal, setMachinParcSignal] = useState(0);
            ...
            <div className="flex flex-col h-screen bg-white text-gray-800 font-sans overflow-hidden" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
                        ...
                        <nav className="relative w-64 max-w-[80vw] bg-white shadow-2xl h-full overflow-y-auto py-4 px-3 flex flex-col gap-0.5 animate-in slide-in-from-left duration-200">
                            ...
                            {(() => {
                                const allItems: Record<string, { label: string; icon: React.ReactNode; active: string }> = {
                                    ...
                                    parametres: { label: 'Paramètres', icon: <Layers className="w-4 h-4" />, active: 'bg-violet-50 border-violet-100 text-violet-700' },
                                    pageMachine: { label: 'Page Machine', icon: <Activity className="w-4 h-4" />, active: 'bg-fuchsia-50 border-fuchsia-100 text-fuchsia-700' },
                ...
                <main className="flex-1 min-h-0 min-w-0 overflow-hidden relative flex flex-col bg-[#fafafa]">
                    ...
                    {currentView === 'parametres' && (
                        <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar bg-[#fafafa]">
                            <div className="px-4 sm:px-8 max-w-6xl mx-auto pb-24 pt-6">
                                <Machin
                                    machines={machines}
                                    ...
                                    onSaveMachineInstance={handleSaveMachineInstance}
                                    onDeleteMachineInstance={handleDeleteMachineInstance}
                                    speedFactors={speedFactors}
                                    ...
                                    parcMachinesOpenSignal={machinParcSignal}
                                    onPurgeAllMachineClasses={handlePurgeAllMachines}
                                />
                                <NavMenuSettings
                                    navConfig={navConfig}
                                    ...
                                    navLabels={{
                                        dashboard: 'Tableau de bord', ingenierie: t.ingenierie, library: t.library,
                                        coupe: 'La Coupe', effectifs: t.effectifs, gestionRh: 'Gestion RH',
                                        planning: 'Planning', suivi: 'Suivi P°', rendement: 'Rendement',
                                        magasin: 'Magasin', export: 'Stock Fini', config: t.configuration,
                                        parametres: 'Paramètres', pageMachine: 'Page Machine', admin: t.admin,
                                    }}
                    ...
                    {currentView === 'pageMachine' && (
                        <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden no-scrollbar bg-[#fafafa]">
                            <PageMachine
                                planningEvents={planningEvents}
                                models={models}
                                settings={globalSettings}
                                ...
                                defaultFleetActorName={(user?.name || user?.email || '').trim()}
                                onSaveMachine={handleSaveMachine}
                                onArchiveMachine={handleArchiveMachine}
                                onToggleMachine={handleToggleMachine}
                                onOpenMachineCatalog={() => {
                                    setMachinParcSignal(s => s + 1);
