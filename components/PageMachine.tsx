...
export default function PageMachine({ 
  ...
  const renderOverviewMap = () => {
    ...
    return (
      ...
      {filteredChains.map((chain) => {
        ...
        return (
          <div 
            ...
            className="group cursor-pointer bg-white rounded-3xl shadow-sm border border-slate-100/80 hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-300 flex flex-col h-full relative"
          >
            <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none z-0">
               {activeData && (
                  <div className="absolute -top-10 -right-10 w-32 h-32 bg-emerald-400/10 rounded-full blur-2xl group-hover:bg-emerald-400/20 transition-all duration-500" />
               )}
            </div>
            ...
            <div className="p-6 flex flex-col h-full relative z-10">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h3 className="font-bold text-slate-800 text-xl tracking-tight mb-2">{chain.name}</h3>
                  {activeData ? (
                    ...
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-50 text-slate-500 text-[11px] font-bold tracking-wide">
                      <div className="w-1.5 h-1.5 rounded-full bg-slate-300" /> Disponible
                    </span>
                  )}
                </div>
                
                <div className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 font-bold text-sm border border-slate-100/50">
            ...
            {activeData && activeModel ? (
              <div className="flex flex-col flex-1 justify-end relative z-10">
                <div className="flex gap-4 items-center mb-5">
                   {activeModel.image ? (
                     <img src={activeModel.image} className="w-16 h-16 rounded-2xl object-cover shadow-sm border border-slate-100" />
                   ) : (
                   ...
                   <div className="flex-1 min-w-0">
                     <p className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest mb-1">{activeModel.ficheData?.client || activeData.clientName || 'Client N/A'}</p>
                     <p className="font-bold text-slate-800 text-base truncate">{activeModel.meta_data.nom_modele}</p>
                     <p className="text-xs font-medium text-slate-500 truncate mt-0.5">Réf: {activeModel.meta_data.reference}</p>
                     <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 flex items-center gap-1">
                       <Clock className="w-3 h-3"/> Lancement: {(activeData.startDate || '').split('T')[0]}
                     </p>
                   </div>
                </div>
                ...
                <div className="space-y-2 mt-auto">
                   <div className="flex justify-between text-[11px] font-bold uppercase tracking-wide">
                      ...
                      <span className="text-slate-700">{qteProduite} / {qteTotal}</span>
                   </div>
              ...
              <div className="flex flex-col flex-1 items-center justify-center text-slate-300 pb-4 relative z-10">
                 <ActivitySquare className="w-8 h-8 mb-3 opacity-50 text-slate-200" />
                 <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Aucun modèle</span>
              </div>
            )}
            
              <div className="mt-4 pt-4 border-t border-slate-100/60 flex items-center overflow-x-auto pb-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                 <div className="flex items-center gap-2 shrink-0">
                   {Object.entries(getMachineBreakdown(chain.id).breakdown).map(([name, count]) => (
                     <div key={name} className="flex items-center gap-2 bg-slate-50 border border-slate-200/60 rounded-lg pl-2.5 pr-1 py-1 shrink-0 hover:bg-slate-100 transition-colors">
                       <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{name}</span>
                       <span className="w-5 h-5 rounded-md bg-white border border-slate-100 flex items-center justify-center text-[10px] font-black text-indigo-600 shadow-sm">{count}</span>
                     </div>
                   ))}
                   {getMachineBreakdown(chain.id).total === 0 && (
                     <span className="text-[10px] font-bold text-slate-400">Aucun matériel affecté</span>
                   )}
                 </div>
              </div>
            </div>
      ...
      <div 
        ...
        className="group cursor-pointer bg-slate-900 rounded-3xl shadow-sm border border-slate-800 hover:shadow-xl hover:shadow-indigo-500/10 transition-all duration-300 flex flex-col h-full relative"
      >
        <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none z-0">
          <div className="absolute -top-10 -right-10 w-32 h-32 bg-indigo-500/20 rounded-full blur-2xl group-hover:bg-indigo-500/30 transition-all duration-500" />
        </div>
        
        <div className="p-6 flex flex-col h-full relative z-10">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h3 className="font-bold text-white text-xl tracking-tight mb-2">Magasin Central</h3>
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-indigo-500/20 text-indigo-300 text-[11px] font-bold tracking-wide border border-indigo-500/30">
          ...
          <div className="flex flex-col flex-1 justify-end">
            <div className="space-y-3">
               <div className="flex justify-between items-center bg-slate-800/50 p-3 rounded-xl border border-slate-700/50">
                 ...
                 <span className="text-sm font-black text-white">{okC}</span>
               </div>
               <div className="flex justify-between items-center bg-slate-800/50 p-3 rounded-xl border border-slate-700/50">
                 ...
                 <span className="text-sm font-black text-white">{brokenCount}</span>
               </div>
            </div>
          </div>
          
          <div className="mt-4 pt-4 border-t border-slate-700/50 flex items-center overflow-x-auto pb-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
             <div className="flex items-center gap-2 shrink-0">
               {Object.entries(getMachineBreakdown(null).breakdown).map(([name, count]) => (
                 <div key={name} className="flex items-center gap-2 bg-slate-800/50 border border-slate-700/80 rounded-lg pl-2.5 pr-1 py-1 shrink-0 hover:bg-slate-800 transition-colors">
                   <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{name}</span>
                   <span className="w-5 h-5 rounded-md bg-slate-800 border border-slate-700 flex items-center justify-center text-[10px] font-black text-indigo-400 shadow-sm">{count}</span>
                 </div>
               ))}
  ...
  return (
    <div className="w-full min-h-screen bg-[#fafcff] text-slate-900 pb-32 font-sans selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <div className="bg-white border-b border-slate-200/50 sticky top-0 z-30 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.03)]">
          ...
          <div className="flex items-center gap-4 md:gap-6">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center shadow-sm shrink-0">
                 <Component className="w-4 h-4 text-white" strokeWidth={2.5} />
              </div>
              <h1 className="text-base font-black text-slate-900 tracking-tight hidden sm:block">Inventaire</h1>
            </div>

            <div className="h-5 w-px bg-slate-200 hidden md:block" />

            <div className="hidden lg:flex items-center gap-4 text-[10px] font-bold tracking-widest uppercase mt-0.5">
               <span className="text-slate-400">Total <span className="text-slate-800 ml-1 font-black text-[11px]">{fleetStats.total}</span></span>
               <span className="text-emerald-500">Prêtes <span className="text-emerald-700 ml-1 font-black text-[11px]">{fleetStats.ok}</span></span>
               <span className="text-rose-500">Panne <span className="text-rose-700 ml-1 font-black text-[11px]">{fleetStats.panne}</span></span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4 flex-1 justify-end">
            ...
            {(selectedChainId || viewingModelId) && (
               <button 
                 ...
                 className="flex items-center gap-1.5 mr-auto px-3 py-1.5 bg-slate-50 text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-lg text-[11px] font-black uppercase tracking-widest transition-colors border border-slate-200/50 shadow-sm"
               >
            ...
            <>
              <div className="flex items-center gap-0.5 bg-slate-50/80 p-0.5 rounded-lg border border-slate-100/50 overflow-x-auto hide-scrollbar shrink-0">
                  {[
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-md text-[11px] font-bold transition-all whitespace-nowrap ${
                        activeTab === tab.id 
                          ? 'bg-white text-slate-900 shadow-[0_1px_4px_rgba(0,0,0,0.05)] border border-slate-200/50' 
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                   <div className="relative hidden xl:block w-48">
                      ...
                      <input 
                        ...
                        placeholder="Filtrer..." 
                        className="w-full bg-slate-50 border border-slate-200/80 rounded-lg py-1.5 pl-8 pr-3 text-[11px] font-bold text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:bg-white transition-all"
                      />
                   </div>
                   
                   <button onClick={() => { setEditingInstance(null); setInstanceEditorOpen(true); }} className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-indigo-600 text-white text-[11px] font-black rounded-lg shadow-sm hover:bg-indigo-700 transition-colors tracking-wide">
